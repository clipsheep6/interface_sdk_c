...
/**
 * @brief Set the desiredPersentTimestamp of a native window buffer.
 *
 * @syscap SystemCapability.Graphic.Graphic2D.NativeWindow
 * @param buffer Indicates the pointer to an <b>OHNativeWindowBuffer</b> pointer.
 * @param desiredPersentTimestamp Indicates the desired time to present the buffer, which should be generated by std::chrono in nanoseconds.
 * @return {@link NATIVE_ERROR_OK} 0 - Success.
 *     {@link NATIVE_ERROR_INVALID_ARGUMENTS} 40001000 - buffer is NULL.
 * @since 12
 * @version 1.0
 */
int32_t OH_NativeWindow_SetBufferDesiredPresentTimestamp(OHNativeWindowBuffer *buffer, 
    int64_t desiredPersentTimestamp);
#ifdef __cplusplus
}
#endif
